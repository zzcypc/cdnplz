...
class cdnplz {
  constructor(options) {
    ...
    if (
      !this.checkOption("tpl_path") ||
      !this.checkOption("static_path") ||
      !this.checkOption("cdn_provider")
    ) {
      process.exit(1);
    }
    ...
    try {
      //读取cdn.cache文件，返回一个json格式文件，key: md5, value: cdn 地址
  ...
  getSubResource(fileName) {
    ...
    regexObj.regexes.forEach(regex => {
      while ((resource = regex.exec(fileContent))) {
        const src = resource[regexObj.group];
        if (!this.isUrl(src) && src !== "") {
          //若是url，则不处理
          subResource.push({
  ...
  dealSubResource(res) {
    ...
    var promises = res.subResource.map(subres => {
      return subres.subResource
        ? this.dealSubResource(subres)
        : this.uploadFile(subres.fileName);
    });
    ...
    return Promise.all(promises).then(response => {
      ...
      if (this.isCSSFile(res.fileName)) {
        // 替换CSS文件中的资源地址
        var cssContent = fs.readFileSync(
          this.getFullPath(res.fileName),
          "utf8"
        );
  ...
  saveFile(fileName, fileContent) {
    ...
    fs.writeFileSync(
      outputFile,
      fileContent,
      this.options.file_encoding || "utf8"
    );
  }
  ...
  replace(fileContent, subResCdnUrl) {
    ...
    subResCdnUrl.forEach(subRes => {
      for (var subResFileName in subRes) {
        ...
        var replaceFileName = subResFileName.substring(
          subStart,
          subResFileName.length
        );
        fileContent = fileContent.replace(
          new RegExp(replaceFileName, "ig"),
          subRes[subResFileName]
        );
      }
    });
    return fileContent;
  ...
  uploadFile(fileName) {
    ...
    if (this.uploadingPromises[fileName]) {
      //判断本次是否已经上传过
    ...
    try {
      ...
      uploadPromise = this.cdnProvider.upload(
        fileName,
        this.options.cdn_options
      );
      this.uploadingPromises[fileName] = uploadPromise;
      return uploadPromise;
    } catch (e) {
  ...
  getRegexes(type) {
    ...
    var jadeRegs = types.map(
      type =>
        new RegExp(
          `${type}(\\s|\\()*(.*?)${tplTypes[type]}(\\s|'|"|\\=)*(.*?)('|").*\\)`,
          "ig"
        )
    );
    var htmlRegs = types.map(
      type =>
        new RegExp(
          `<${type}(\\s)+(.*?)${tplTypes[type]}\\s*=\\s*('|")(.*?)('|").*?`,
          "ig"
        )
    );
  ...
  getTplFileNames() {
    if (this.tplSuffixs.length == 1) {
      return glob.sync(`${this.options.tpl_path}/**/*.${this.tplSuffixs[0]}`, {
        mark: true
      }); // 命中的模板文件
    }
